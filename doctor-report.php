...
if(isset($_POST['viewButton']))
{
	...
	else
	{
		...
		$detailsMessage .= "<h2>Product Bills</h2>";
		$query = mysqli_query($conn, "SELECT * FROM ProductBills WHERE PatientID = '$patientInput' AND DueDate >= CURDATE() ORDER BY DueDate");
		$result = mysqli_fetch_assoc($query);
		
		if(!$result)
		{
			$detailsMessage .=	"<div class = \"infoBox doctor\">
									<span>This patient doesn't have any product bills to pay.</span>
								</div>";
		}
		else
		{
		...
		$detailsMessage .= "<h2>Service Bills</h2>";
		...
		{
			$detailsMessage .=	"<div class = \"infoBox doctor\">
									<span>This patient doesn't have any service bills to pay.</span>
								</div>";
		}
		else
		{
		...
		$detailsMessage .= "<h2>Test Bills</h2>";
		$query = mysqli_query($conn, "SELECT * FROM TestBills WHERE PatientID = '$patientInput' AND DueDate >= CURDATE() ORDER BY DueDate");
		$result = mysqli_fetch_assoc($query);
		
		if(!$result)
		{
			$detailsMessage .=	"<div class = \"infoBox doctor\">
									<span>This patient doesn't have any test bills to pay.</span>
								</div>";
		}
		else
		{
